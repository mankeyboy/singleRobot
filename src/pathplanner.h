...
// Returns the nearest traversable point for any point p
Point nearest(int **map, int X_MAX, int Y_MAX, int SCALE, Point p)
{
  if(passable(map, X_MAX, Y_MAX, SCALE, p))
  {
    return p;
  }
  double radius=1;
  while(1)
  {
    for(int x=p.first-radius;x<=p.first+radius;x++)
    {
      for(int y=p.second-radius;y<=p.second+radius;y++)
      {
        ...
        {
          if(passable(map, X_MAX, Y_MAX, SCALE, Point(x,y)))
          {
            return Point(x, y);
          }
        }
...
std::vector<Point> AStar(int **map, int X_MAX, int Y_MAX, int SCALE, Point source, Point destination) {
  ...
  source = nearest(map, X_MAX, Y_MAX, SCALE, source);
  destination = nearest(map, X_MAX, Y_MAX, SCALE, destination);
